...
public final class ViolationService {
    ...
    public ViolationService(CortexAntiCheatPlugin plugin, PunishmentService punishmentService) {
        ...
        this.mlAnomalyService = new MLAnomalyService(plugin);
    }
    ...
    public void flag(DetectionContext context) {
        ...
        sendAlert(context, profile.violations());

        MLAnomalyService.MLResult mlResult = mlAnomalyService.evaluate(context, profile);
        if (mlResult.anomalous()) {
            profile.addViolation(mlResult.bonusViolation());
            DetectionContext mlContext = new DetectionContext(context.player(), "ML.Anomaly", mlResult.bonusViolation(), mlResult.detail());
            sendAlert(mlContext, profile.violations());
        }
    ...
    private void sendAlert(DetectionContext context, double violations) {
        String alert = ChatColor.RED + "[Cortex] " + ChatColor.YELLOW + context.player().getName()
                ...
                + ChatColor.GRAY + " (VL " + String.format("%.1f", violations) + ") "
        ...
        plugin.discordWebhookService().logViolation(context.player().getName(), context.check(), violations, context.detail());
    }
}
